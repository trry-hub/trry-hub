...
Babel是一个广泛使用的 ES6 转码器，你可以将 ES6 代码转为 ES5 代码，从而在老版本的浏览器执行。这意味着，你可以用 ES6 的方式编写程序，又不用带薪现在环境是否支持。下面是一个例子。
...
下面的命令在项目目录中，安装了 Babel。

```bash
$ npm insatll --save-dev @babel/core
```

### 配置文件.babelrc

Babel 的配置文件是```.babelrc```,存放在项目的目录下。使用 Babel 的第一步，就是配置这个文件。
...
```@babel/node```模块的```babel-node```命令，提供一个支持ES6的 REPL 环境。它支持Node 的 REPL 环境的所有功能，而且可以直接运行ES6代码。
